# Current proof audit — scoped precision repair

Source: Rebased_99afee22. Authorized list: original root PROOF_AUDIT(1).md I1–I10.

**Scoped repair verdict: PASS (provisional). Full manuscript verdict: WARN.**

This file supersedes earlier audit conclusions only for the exact hashes below. The original root and nested audits remain historical evidence. No unrestricted full-manuscript or cross-family acceptance is claimed. Before-review evidence is in prompts/26091901-01/review_before.md; after-review evidence follows.
...
## Deterministic and delivery checks

The final manuscript compiles to 38 pages and the before/after report to 3 pages. Duplicate labels: zero. Unresolved references: zero. No overfull boxes or fatal LaTeX errors. Original theorem/equation labels are preserved; only an unused second-table label is removed. Numbered display math is identical modulo the call-count font normalization. Experimental numerals and protected data/code/reference files are unchanged. E3 active-PDF changes are confined to its two title text regions. Full details are in verification/proof_precision_20260919/ at the package root.

The optional render-html skill was not available; HTML auto-rendering was skipped as non-blocking. The Markdown and compiled PDF reports are provided.
