...
\]
Under the reference-isolated comparison specified in \Cref{prop:reference-sensitivity}, the two trajectories have the same wealth path.  Iterating the preceding inequality for $h$ steps proves \eqref{eq:reference-contraction}.  Since their terminal wealth is common,
...
\begin{proof}
Fix the episode $k$ of \Cref{thm:gradient-identity}.  We prove the gain contribution to $g_k(\theta)$; the loss contribution follows by the same calculation and is subtracted at the end.  It is enough to identify $\nabla S_\theta^+(y)$ and justify moving the derivative through the gain-side CPT integral.
