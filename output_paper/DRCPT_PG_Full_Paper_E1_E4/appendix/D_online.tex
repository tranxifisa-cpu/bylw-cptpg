...
Write $A=A_{\rho,W}$.  With the convention $Q_t(\theta_t)=0$ for $t\le0$, the coefficients $\rho^j/A$, $j=0,\ldots,W-1$, are nonnegative and sum to one.  Convexity of $x\mapsto\norm{x}_2^2$ gives
\[
 \norm{\bar Q_k}_2^2
 \le \frac1A\sum_{j=0}^{W-1}\rho^j
...
\begin{align*}
 ...
 &\le
 \sum_{t=1}^{K}\zeta_t
 \left\{\frac1A\sum_{j=0}^{\min\{W-1,K-t\}}\rho^j\right\}
...
\subsection{Independent-stream residual evaluation}
