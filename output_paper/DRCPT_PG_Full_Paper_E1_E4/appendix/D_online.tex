...
\subsection{Square-first dynamic local regret}

\begin{proof}[Proof of \Cref{cor:dynamic-local-regret}]
By definition,
\[
 \Reg_{\rho,w}(K)
 =\sum_{k=1}^{K}\frac{1}{W_{\rho,m_k}}
   \sum_{j=0}^{m_k-1}\rho^j\zeta_{k-j}.
\]
All terms are nonnegative and $W_{\rho,m_k}\ge1$.  Reindexing by the episode $t=k-j$ that generated each squared residual therefore gives
\begin{align*}
 \Reg_{\rho,w}(K)
 &\le
 \sum_{t=1}^{K}\zeta_t
 \sum_{j=0}^{\min\{w-1,K-t\}}\rho^j\notag\\
 &\le W_{\rho,w}\sum_{t=1}^{K}\zeta_t.
\end{align*}
This proves \eqref{eq:dynamic-local-regret-bound}.  Taking expectations and dividing by $K$ gives \eqref{eq:average-dynamic-local-regret-bound}.  The factor $W_{\rho,w}$ is fixed whenever the window design $(w,\rho)$ is fixed, so it does not alter the vanishing-rate conclusion.
\end{proof}
