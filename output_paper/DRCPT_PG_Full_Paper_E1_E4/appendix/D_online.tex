...
Write $A=A_{\rho,W}$.  With the convention $Q_t(\theta_t)=0$ for $t\le0$, the coefficients $\rho^j/A$, $j=0,\ldots,W-1$, are nonnegative and sum to one.  Convexity of $q\mapsto\norm{q}_2^2$ gives
\[
 \norm{\bar Q_k}_2^2
...
$\cV_K=O(\sum_{k<K}\alpha_k)+O(1)=O(K^{1/4})=o(K)$.
