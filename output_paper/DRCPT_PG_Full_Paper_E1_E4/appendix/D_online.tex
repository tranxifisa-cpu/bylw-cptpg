...
\begin{equation}\label{eq:residual-evaluation-lipschitz}
 ...
 \le \vartheta^{-1}\norm{\widehat g-g}_2.
\end{equation}
Hence, if $\widehat g^{(1)},\widehat g^{(2)}$ are independent gradient estimates with
...
\end{equation}
Let $\operatorname{Bias}_j:=\E\widehat Q^{(j)}-\mathcal Q_\theta(g)$ for $j=1,2$.
Independence gives
\[
 \E\ip{\widehat Q^{(1)}}{\widehat Q^{(2)}}
...
$\norm{\operatorname{Bias}_j}_2\le\varepsilon_g/\vartheta$ for $j=1,2$.  Since
$\norm{\mathcal Q_\theta(g)}_2\le1$, because $\norm{\mathsf d(g)}_2\le1$,
\[
 \left|
 \ip{\mathcal Q_\theta(g)+\operatorname{Bias}_1}
 {\mathcal Q_\theta(g)+\operatorname{Bias}_2}
...
For the online path through period $K$, let $\mathcal C_K$ be the sigma-field generated by the realized iterates and episode targets, before the independent evaluation draws.  Suppose the two complete evaluation streams are conditionally independent given $\mathcal C_K$.  For $1\le t\le K$, let $\widehat g_t^{(j)}$ be the held-out gradient estimate in stream $j=1,2$, and define
\[
...
\]
where $\varepsilon_{g,t}$ is a nonnegative $\mathcal C_K$-measurable error bound.  Set $\widehat Q_t^{(j)}=0$ and $\varepsilon_{g,t}=0$ for $t\le0$, consistently with \eqref{eq:dynamic-window-residual}.  Define
\[
 \widehat{\bar Q}_k^{(j)}
...
\]
for $1\le k\le K$.  Conditional Jensen's inequality and \eqref{eq:residual-evaluation-lipschitz} bound the mean error of each smoothed stream by $\bar\varepsilon_k/\vartheta$.  Since $\norm{\bar Q_k}_2\le1$, conditional independence and the same cross-product argument give
...
\end{equation}
Taking expectations also bounds the difference between the expected cross-product and $\E\norm{\bar Q_k}_2^2$ by the expectation of the right-hand side.
