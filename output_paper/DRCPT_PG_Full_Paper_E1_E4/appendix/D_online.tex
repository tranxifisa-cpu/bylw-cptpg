...
With the convention $Q_t(\theta_t)=0$ for $t\le0$, the coefficients
$\rho^j/\sum_{r=0}^{W-1}\rho^r$, $j=0,\ldots,W-1$, are nonnegative and sum to one.  Convexity of $x\mapsto\norm{x}_2^2$ gives
\[
 \norm{\bar Q_k}_2^2
 \le
 \frac{\sum_{j=0}^{W-1}\rho^j
...
\begin{align*}
 ...
 \sum_{t=1}^{K}\zeta_t
 \left\{
 \frac{\sum_{j=0}^{\min\{W-1,K-t\}}\rho^j}
      {\sum_{r=0}^{W-1}\rho^r}
 \right\}
...
\end{equation}
Indeed, independence gives
$\E\ip{\widehat Q^{(1)}}{\widehat Q^{(2)}}=\norm{\E\widehat Q^{(1)}}_2^2$.
Writing $e=\E\widehat Q^{(1)}-\mathcal Q_\theta(g)$, Jensen's inequality and
\eqref{eq:residual-evaluation-lipschitz} give $\norm e_2\le\varepsilon_g/\vartheta$.
Finally $\norm{\mathcal Q_\theta(g)}_2\le1$, because $\norm{\mathsf d(g)}_2\le1$, which yields \eqref{eq:cross-stream-residual-error}.

The same argument passes through the linear DLR smoothing.  If
\[
 \widehat{\bar Q}_k^{(j)}
 =\frac{\sum_{r=0}^{W-1}\rho^r\widehat Q_{k-r}^{(j)}}
 {\sum_{r=0}^{W-1}\rho^r},
 \qquad
 \bar\varepsilon_k
 =\frac{\sum_{r=0}^{W-1}\rho^r\varepsilon_{g,k-r}}
 {\sum_{r=0}^{W-1}\rho^r},
\]
