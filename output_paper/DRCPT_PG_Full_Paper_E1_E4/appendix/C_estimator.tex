...
Throughout this appendix, condition on one frozen target and let $g$ denote its CPT gradient at the queried parameter $\theta$.  The complete trajectory summaries $\mathsf X_1,\mathsf X_2,\ldots$ are independent and identically distributed under this conditional law.
...
Under Assumptions \ref{ass:trajectory}--\ref{ass:cpt-regularity}, fix one frozen target and parameter, and let $g$ denote its CPT gradient.  There exist a measurable $\varphi$ and constants $C_{\rm rem},C_U<\infty$ such that
\begin{equation}\label{eq:loo-expansion}
 U_n^{\rm raw}-g
 =\frac1n\sum_{i=1}^{n}\varphi(\mathsf X_i)+\mathcal B_n,
 \qquad
 \E\varphi(\mathsf X_i)=0,
 \qquad
...
\end{align*}
By \eqref{eq:score-gradient-identity} and $\E G=0$, $\E \varphi_0(\mathsf X)=g$.  Moreover
...
\[
 \varphi_1(x')=\E_{\mathsf X}\{\mathcal K(\mathsf X,x')\}.
\]
Then $\E \varphi_1(\mathsf X')=0$.  Put
$\mathcal K^\circ(x,x')=\mathcal K(x,x')-\varphi_1(x')$.  Both first-order projections of $\mathcal K^\circ$ vanish:
\[
...
\begin{equation}\label{eq:ordered-hoeffding}
 ...
 =\frac1n\sum_{j=1}^{n}\varphi_1(\mathsf X_j)
 +\overline{\mathcal K}_n^\circ,
\end{equation}
where
\[
 \overline{\mathcal K}_n^\circ=\frac{1}{n(n-1)}\sum_{i\ne j}\mathcal K^\circ(\mathsf X_i,\mathsf X_j).
\]
...
\begin{equation}\label{eq:degenerate-remainder}
 \E\norm{\overline{\mathcal K}_n^\circ}_2^2\le\frac{C_D}{n^2}
\end{equation}
...
\end{equation}
for a universal finite $C_e$.  By \eqref{eq:weight-taylor}, Cauchy--Schwarz in the $y$-integral, independence of $G_i$ from the leave-one-out empirical survival, and $\E\norm{G_i}_2^2<\infty$, the $i$th Taylor contribution has second moment at most $C_T/n^2$.  Jensen's inequality for the average over $i$ then gives a second-moment bound of the same order for the averaged Taylor remainder $\mathcal B_n^{\rm Tay}$:
\begin{equation}\label{eq:taylor-remainder-bound}
 \E\norm{\mathcal B_n^{\rm Tay}}_2^2\le\frac{C_T}{n^2}.
\end{equation}
...
\[
 ...
 =\frac1n\sum_{i=1}^{n}\varphi(\mathsf X_i)+\mathcal B_n,
 \qquad
 \varphi=\varphi_0-g+\varphi_1,
 \qquad
 \mathcal B_n=\overline{\mathcal K}_n^\circ+\mathcal B_n^{\rm Tay}.
\]
The vector function $\varphi$ has mean zero, and \eqref{eq:degenerate-remainder}--\eqref{eq:taylor-remainder-bound} imply
...
\[
 U_{n_\ell}^{\rm raw}
 =g+\frac1{n_\ell}\sum_{i=1}^{n_\ell}\varphi(\mathsf X_i)+\mathcal B_{n_\ell},
\]
and
\[
 \frac12\{U_{n_\ell/2}^{\rm raw,(1)}+U_{n_\ell/2}^{\rm raw,(2)}\}
 =g+\frac1{n_\ell}\sum_{i=1}^{n_\ell}\varphi(\mathsf X_i)
...
$2B_vL_f/(n-1)$.  Hence, for a deterministic constant $C_*$ depending only on $B_v,C_1,C_2$,
\begin{align*}
...
Let $g$ denote the frozen target gradient.  For $n\ge2$,
...
\begin{equation}\label{eq:loo-bias-order}
 \norm{\bar U_n-g}_2\le \frac{C_{\rm bias}}{n}.
\end{equation}
\end{lemma}
...
It remains to control $\bar U_n-g$.  This follows from the expansion in \Cref{lem:level-cancellation}.  Taking expectations in \eqref{eq:loo-expansion} and using $\E\varphi(\mathsf X)=0$ yields
\[
 \norm{\bar U_n-g}_2
...
\]
Thus \eqref{eq:loo-bias-order} holds with $C_{\rm bias}=\sqrt{C_{\rm rem}}$.
\end{proof}
...
For fixed target and fixed $(n,p_{\rm act},b)$, \Cref{thm:estimator} gives iid vectors $Z_{n,r}^{\db}$ with mean $g$ and finite covariance $\Sigma_n$.  For any fixed $t\in\R^d$, the scalar variables
$t^\top(Z_{n,r}^{\db}-g)$ are iid with finite variance.  The classical scalar central limit theorem therefore gives
\[
