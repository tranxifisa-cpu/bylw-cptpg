...
\begin{figure}[H]
    ...
    \caption{Robustness of the online-tracking advantage measured by the configuration-matched post-change DLR ratio $\mathrm{Ratio}_{\mathrm{DLR}}$.  \textbf{A}: reference-adaptation surface over the simulated $9\times9$ grid; the black contour marks a ratio of one, the diagonal marks $\eta_+=\eta_-$, and the star marks the default asymmetric rule.  \textbf{B}: horizon sweep with paired-seed 95\% bootstrap intervals; the horizontal line marks a ratio of one and the vertical line marks $h=5$.  \textbf{C}: diagnostically screened $(\gamma,\vartheta)$ grid; blank cells lie outside $\gamma L_{\rm diag}\le\vartheta$, and the star marks the E3 default.}
    \label{fig:e4-robustness}
\end{figure}
\FloatBarrier

\paragraph{Results.}
E4 asks whether the E3 separation is a property of the mechanism or an accident of one tuning choice.  The reference sweep gives the clearest answer.  Continued updating has lower post-change DLR in $76$ of $81$ reference configurations, and all $36$ gain-dominant asymmetric settings with $\eta_+>\eta_-$ lie below the matched-control boundary; $33$ of those have paired 95\% intervals entirely below one.  The default rule has $\mathrm{Ratio}_{\mathrm{DLR}}=0.458$ with interval $[0.332,0.621]$.  The surface also identifies a coherent boundary of the phenomenon: strongly loss-dominant adaptation can reverse the comparison, as at $(\eta_+,\eta_-)=(0.05,0.40)$ where the ratio is $1.403$ with interval $[1.224,1.552]$ (\Cref{fig:e4-robustness}A).  The robustness region therefore follows the qualitative reference dynamics rather than looking like isolated favorable cells.

The other two sweeps test whether the effect is tied to discretization or optimizer tuning.  Mean ratios are below one at all $11$ horizons, with $10$ intervals entirely below one; only the $h=50$ interval crosses one, so the E3 result is not specific to the five-day episode length (\Cref{fig:e4-robustness}B).  Across the $48$ screened $(\gamma,\vartheta)$ pairs, $47$ mean ratios are below one and $45$ intervals are entirely below one; the lone point estimate above one has interval $[0.876,1.342]$ and is unresolved from the matched control.  Taken together, the sweeps map a broad region in which continued adaptation improves first-order tracking.  This is stronger evidence than a single default run because it shows that the E3 separation survives changes to the behavioral state dynamics, the temporal aggregation scale, and the optimizer itself.
