...
\end{equation}
This is an admissible portfolio trajectory model under the common bounds in Assumption \ref{ass:minimax-oracle}.  The loss-side value is identically zero.
...
$P(\mathsf U>t)=1-t^{e^{\theta^{[1]}}}$, a change of variables $y=v(\upsilon t)$ in the gain-side CPT integral gives
...
\begin{align*}
 ...
 &\ge (d_W-2C_2p)A_v.
\end{align*}
Choose $p_{\max}\in(0,1/2]$ small enough that $2C_2p_{\max}\le d_W/2$; if $C_2=0$, any choice in $(0,1/2]$ is admissible.  Let
\[
 p_*=p_{\max}/2,
 \qquad
...
$\{\mathsf T_{\rm call}\ge j\}$; these events are measurable before the $j$th fresh return is observed.  The increments are uniformly integrable because $p_\pm$ stay in the compact interior interval $\mathcal I$ and $\E_+\mathsf T_{\rm call}<\infty$.

Choose $\delta_*$ small enough that $C_I\delta_*^2\le1/8$.  Pinsker's inequality then gives
\begin{equation}\label{eq:minimax-tv}
 \operatorname{TV}(\mathbb P_+^{\rm tr},\mathbb P_-^{\rm tr})\le\frac14.
\end{equation}
...
\begin{equation}\label{eq:minimax-target-distance}
 ...
 \ge2c_*\delta.
\end{equation}
Given any estimator $\widehat g$, define the test that chooses the closer of the two targets.  On a testing error, the estimator must be at least $D_{\mathsf B}^g/2$ from the true target.  The sum of the two testing error probabilities is at least one minus the total variation distance.  Therefore
\begin{align*}
 \max_{s\in\{+,-\}}
 \E_s\norm{\widehat g-\mathfrak g(p_s)e_1}_2^2
 &\ge \frac{(D_{\mathsf B}^g)^2}{8}
 \left\{1-\operatorname{TV}(\mathbb P_+^{\rm tr},\mathbb P_-^{\rm tr})\right\}\\
...
Fix any admissible estimator design parameters $(n,\varrho,b)$ from \Cref{thm:estimator}.  The common model-class constants in Assumption \ref{ass:minimax-oracle} make
\[
 \sup_{P\in\mathfrak C}\tr\Cov_P(Z_n^{\db})\le C_{\rm var}(n)<\infty,
 \qquad
 C_{\rm cost}=n(1+\varrho \Upsilon_b)<\infty
\]
