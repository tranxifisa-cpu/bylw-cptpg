...
For a realization $x=(v^+,v^-,g_x)$, write $G_x=g_x$ and $I_{x,\sigma}(y)=\1\{v^\sigma>y\}$.
...
\begin{equation}\label{eq:weight-taylor}
 ...
 +r_\sigma(e),
 \qquad
 \abs{r_\sigma(e)}\le\frac{C_3}{2}e^2.
\end{equation}
...
\end{align*}
By \eqref{eq:score-gradient-identity} and $\E G=0$, $\E h_0(\mathsf X)=g$.  Moreover
...
\[
 h_1(x')=\E_{\mathsf X}\{\mathcal K(\mathsf X,x')\}.
\]
Then $\E h_1(\mathsf X')=0$.  Put
$\mathcal K^\circ(x,x')=\mathcal K(x,x')-h_1(x')$.  Both first-order projections of $\mathcal K^\circ$ vanish:
\[
...
\begin{equation}\label{eq:ordered-hoeffding}
 ...
 =\frac1n\sum_{j=1}^{n}h_1(\mathsf X_j)
 +\mathcal D_n,
\end{equation}
where
\[
...
\end{equation}
for a universal finite $C_e$.  By \eqref{eq:weight-taylor}, Cauchy--Schwarz in the $y$-integral, independence of $G_i$ from the leave-one-out empirical survival, and $\E\norm{G_i}_2^2<\infty$, the $i$th Taylor contribution has second moment at most $C_T/n^2$.  Jensen's inequality for the average over $i$ then gives a second-moment bound of the same order for the averaged Taylor remainder $T_n$:
\begin{equation}\label{eq:taylor-remainder-bound}
 \E\norm{T_n}_2^2\le\frac{C_T'}{n^2}.
\end{equation}
...
\[
 ...
 \qquad
 \mathfrak h=h_0-g+h_1,
 \qquad
 \mathcal B_n=\mathcal D_n+T_n.
\]
...
It remains to control $\mu_n-g$.  This follows from the expansion in \Cref{lem:level-cancellation}.  Taking expectations in \eqref{eq:loo-expansion} and using $\E\mathfrak h(\mathsf X)=0$ yields
\[
 \norm{\mu_n-g}_2
...
We first prove exact unbiasedness.  By \Cref{lem:centering-expectation}, the centered base has mean $\mu_n$.  For the raw correction,
\begin{equation}\label{eq:delta-mean}
 \E D_{n,\ell}^{\rm raw}
 =\mu_{n2^\ell}-\mu_{n2^{\ell-1}}.
\end{equation}
...
\]
so the expectation can be interchanged with the infinite randomized series.  Since \eqref{eq:loo-bias-order} gives $\mu_{n2^{\ell_{\max}}}\to g$,
\begin{align*}
 \E Z_n^{\db}
 &=\mu_n+\sum_{\ell\ge1}\E D_{n,\ell}^{\rm raw}\\
 &=\mu_n+\lim_{\ell_{\max}\to\infty}(\mu_{n2^{\ell_{\max}}}-\mu_n)
 =g.
\end{align*}
...
\begin{align*}
 \E\left\|\frac{\mathsf A}{\varrho\nu_{\ell^\star}}D_{n,\ell^\star}^{\rm raw}\right\|_2^2
 &=\frac1\varrho\sum_{\ell\ge1}
 \frac{\E\norm{D_{n,\ell}^{\rm raw}}_2^2}{\nu_\ell}\\
 &\le\frac{9C_{\rm rem}}{\varrho n^2}
 \sum_{\ell\ge1}\frac{4^{-\ell}}{\nu_\ell}
 =\frac{9C_{\rm rem}\Xi_b}{\varrho n^2}.
\end{align*}
...
\begin{align*}
 ...
 &=n+\varrho n\sum_{\ell\ge1}\nu_\ell2^\ell
 =n(1+\varrho \Upsilon_b),
\end{align*}
...
For fixed target and fixed $(n,\varrho,b)$, \Cref{thm:estimator} gives iid vectors $Z_{n,r}^{\db}$ with mean $g$ and finite covariance $\Sigma_n$.  For any fixed $t\in\R^d$, the scalar variables
