...
Value magnitudes use the quadratically smoothed power specification in the manuscript.
...
class CPTPreference:
    ...
    def magnitude(self, x):
        ...
        return (x ** 2 + self.value_epsilon ** 2) ** (self.alpha / 2) - self.value_epsilon ** self.alpha

    def inverse_magnitude(self, z):
        return np.sqrt(np.maximum((np.asarray(z) + self.value_epsilon ** self.alpha) **
                                  (2 / self.alpha) - self.value_epsilon ** 2, 0.0))
