...
class PaperPanel:
    ...
    def load(cls, path, start, end):
        ...
        # Git can normalize Windows CSV line endings. Accept only an exact
        # recorded digest under one of these two reversible newline encodings.
        digests = {hashlib.sha256(blob).hexdigest() for blob in
                   (raw, normalized, normalized.replace(b"\n", b"\r\n"))}
        if metadata["panel_sha256"] not in digests:
            raise ValueError("Prepared panel checksum mismatch (including LF/CRLF check)")
