...
class BetaMarket:
    ...
    action_fraction: float = 1.0

    def __post_init__(self):
        if not (0 <= self.probability <= 1 and 0 < self.return_size < 1):
            raise ValueError("Invalid market law")
        if not 0 < self.action_fraction <= 1:
            raise ValueError("Require a fixed action fraction in (0,1]")
    ...
    def draw(self, rng, theta, size):
        ...
        terminal = self.wealth * (1 + self.action_fraction * stock_weight * returns)
    ...
    def oracle(self, theta, order=128):
        ...
        for sign, scale, eta in ((1, 1.0, self.eta_gain),
            ...
            extent = max((1 - eta) * (self.wealth * (1 + sign * self.action_fraction * self.return_size)
            ...
            for lo, hi in zip(edges[:-1], edges[1:]):
                ...
                for ret, prob in ((self.action_fraction * self.return_size, self.probability),
                                  (-self.action_fraction * self.return_size, 1 - self.probability)):
                ...
                objective += sign * (hi - lo) * (self.cpt.weight(survival, 0 if sign == 1 else 1) @ weights)
                gradient += sign * (hi - lo) * (
                    (self.cpt.weight_prime(survival, 0 if sign == 1 else 1) * derivative) @ weights)
        if theta.ndim == 0:
